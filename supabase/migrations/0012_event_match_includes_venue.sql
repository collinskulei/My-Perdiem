-- Fixes event-matching in import_historical_events() to key on name + venue
-- together, not name alone.
--
-- Found via a real file: some source spreadsheets use a generic training
-- type as the event name ("Workshop/Conference", "TOT", "EUT") and put the
-- actual venue in its own separate column, rather than folding the venue
-- into the name ("Workshop/Conference - Enashipai Resort & SPA") the way
-- earlier files did. Matching by name alone meant every venue sharing a
-- generic name got merged into a single "event" - e.g. one real file had
-- "Workshop/Conference" spanning 7 completely different real venues, all
-- collapsed into one event whose stored venue/training-dates only ever
-- reflected whichever row happened to be processed first.
--
-- Only the event-matching condition changes - nothing else about the
-- function (the gap-filling sync, the new field set, etc. - see
-- 0011_historical_fields_and_sync.sql) is touched. The return shape is
-- unchanged, so a plain CREATE OR REPLACE works (no DROP FUNCTION needed
-- this time).
--
-- Backward-compatible with the older "venue folded into the name"
-- convention: those rows never populate a separate venue column, so
-- v_venue_id is NULL for all of them consistently, and `venue_id is not
-- distinct from v_venue_id` (NULL-safe equality) still groups them
-- together exactly as before.
--
-- Note: this does NOT retroactively fix events already created under the
-- old, too-generic matching - those already-merged events keep their
-- wrong venue/training-date data until the affected historical data is
-- wiped and re-imported under this corrected function.
...
begin
  ...
  loop
    ...
    -- Matches on name AND venue together (see file header) - a generic
    -- training-type name at a different venue is a different event.
