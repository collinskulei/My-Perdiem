-- Follow-up to 0012_event_match_includes_venue.sql, same class of bug: event
-- matching still keyed only on name + venue, and some source files never
-- carry a venue at all - "delegation DSA" / roving-support-staff payments
-- with no fixed training venue, just a generic name like "County
-- activation". Found via a real case: Uasin Gishu, Kakamega, and Nakuru
-- each uploaded their own "County activation"-style DSA batch (different
-- counties, different actual training periods), all with a blank venue and
-- all *paid* on the same date (one shared December disbursement day for
-- everyone). Matching on name + venue alone made every county's batch
-- resolve to the SAME event - and since the identity-match that decides
-- gap-fill-vs-new keys off event_id, roving staff who appear in more than
-- one county's roster (same phone, same payment date, sometimes even the
-- same day-rate) got silently collapsed into one payment instead of one
-- per county, discarding whichever county's amount arrived second.
--
-- Fix: event matching now also requires the same training_start_date (or
-- both blank), not just name + venue. Different counties' rollouts
-- genuinely happen on different training dates even when they're paid out
-- together, so this is normally enough to tell them apart without needing
-- to introduce county as a first-class matching field.
--
-- Known limitation (accepted trade-off, same shape as the Payment Date one
-- already documented below): a row whose Training Start Date was blank on
-- the first upload and filled in later won't be recognized as the same
-- event - it creates a new one instead of gap-filling. Also: this does NOT
-- retroactively fix events already created under the old, too-generic
-- matching (e.g. the Uasin Gishu/Kakamega/Nakuru collision above) - those
-- already-merged payments need manual review/correction; only imports run
-- after this migration get the corrected behavior.
