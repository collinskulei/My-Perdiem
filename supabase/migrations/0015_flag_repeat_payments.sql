-- Follow-up to 0014_dedup_matches_on_amount.sql: that migration stopped a
-- genuine second payment (same participant/event/date/phone, different
-- amount) from being silently merged into the first one - it now inserts
-- as its own row instead. But a plain "2 new records" result gives an admin
-- reviewing the data no signal that anything unusual happened - a repeat
-- payment to the same person for the same event/date looks identical to
-- any other row unless someone happens to notice.
--
-- Fix: when an incoming row identity-matches an existing record (same
-- client/event/date/phone-or-name) but the amount differs, both the new
-- row and the pre-existing one get a `flag_reason` explaining why - "this
-- is a repeat payment, not a duplicate, please verify" - instead of
-- importing silently. A genuine duplicate (identity AND amount both match)
-- still merges via the existing gap-fill path, unflagged, exactly as
-- before.

alter table public.perdiem_requests add column if not exists flag_reason text;
...
begin
  ...
  loop
    ...
    if v_existing_request_id is not null then
      ...
      -- No amount-matching identity record. If an identity match exists at
      -- all, this is a repeat payment (not a duplicate) - flag both this
      -- new row and the pre-existing one so an admin reviewing the data
      -- sees it called out, rather than it looking like an ordinary import.
      v_flag_reason := null;
      if v_identity_match_id is not null then
        v_flag_reason := format(
          'Repeat payment: same participant, event, date and phone/name as another payment of KES %s (this payment: KES %s). Kept as a separate record, not merged - please verify both are legitimate.',
