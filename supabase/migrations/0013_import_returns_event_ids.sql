-- Fixes historical imports timing out on large files (reported: 7000+ rows).
--
-- import_historical_events() runs the whole spreadsheet as ONE atomic
-- transaction, and does several queries per row (venue lookup/insert,
-- event lookup/insert, participant phone match, existing-request lookup
-- for the gap-fill sync, insert/update) - for 7000+ rows that's tens of
-- thousands of statements in a single transaction, well past what
-- Supabase's statement timeout allows. The whole call then fails and
-- rolls back with nothing committed.
--
-- Fix: the admin-historical-import.tsx dialog now splits large imports
-- into batches and calls this RPC once per batch (see that file) - each
-- batch is its own short transaction, well within the timeout. This is
-- safe with the existing gap-filling sync: if one batch fails partway,
-- already-committed earlier batches stay committed, and simply retrying
-- the whole import is safe - already-imported rows are recognized and
-- filled/no-op'd rather than duplicated.
--
-- The only change needed in this function itself: return the actual
-- touched event ids (not a pre-aggregated count) so the client can union
-- them across batches for an accurate final "N events" total instead of
-- summing per-batch counts, which would double-count any event touched by
-- more than one batch.
