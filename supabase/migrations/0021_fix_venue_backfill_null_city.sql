-- Fixes a crash introduced in 0019_backfill_blank_venue_county_on_match.sql
-- (carried forward unchanged into 0020): "null value in column "city" of
-- relation "venues" violates not-null constraint" whenever an import file
-- has no Venue City column/batch default (so every row's venueCity is a
-- blank string, not actually absent) and more than one row shares the same
-- venue name (near-guaranteed - most events have many participants).
--
-- Root cause: on an existing-venue match, the backfill does
--   city = coalesce(nullif(city, ''), nullif(row_data->>'venueCity', ''))
-- If the stored city is already blank and the incoming one is blank too,
-- both sides collapse to SQL NULL via nullif(), so coalesce(NULL, NULL) is
-- NULL and the UPDATE tries to write that - not just a no-op backfill.
-- The WHERE guard has the same loophole in reverse: it checks
-- `row_data->>'venueCity' is not null`, which is true even for an empty
-- string (only an actually-absent JSON key would make it SQL NULL), so it
-- doesn't skip a blank incoming value the way the migration's own comment
-- says it should.
--
-- Fix: coalesce all the way down to the column's own current value (never
-- NULL, even when both sides are blank) and tighten the guard to require a
-- genuinely non-blank incoming value via nullif(..., '') is not null,
-- matching the "only backfill when we actually have something new" intent.
-- No other part of import_historical_events changes.
...
begin
  ...
  loop
    ...
    if v_venue_id is null and row_data->>'venueName' is not null then
      ...
      -- write - see this migration's header for the crash that happened
      -- without that last fallback.
