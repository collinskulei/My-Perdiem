-- The County filter on the Reports and Insights tabs joins through
-- venues.county (events.venue_id -> venues.id -> venues.county). A live-data
-- check found 54 of 59 venues have county = '' - so selecting almost any
-- specific county returns next to nothing even though "All Counties" shows
-- full data.
--
-- Root cause: import_historical_events() matches an existing venue purely by
-- name (`lower(name) = lower(venueName)`) and, on a match, never touches its
-- county/city - only a brand-new venue gets those columns set (see
-- 0006/0012/0018). So whichever import row happened to create a venue first
-- permanently fixes its county, even blank, no matter what later rows for
-- the same venue name supply.
--
-- Fix: on an existing-venue match, backfill county/city from the current
-- row whenever the stored value is blank and the row has a non-blank one.
-- Never overwrites a county/city that's already set (gap-fill semantics,
-- same as every other field this RPC touches) - if a county was wrong, an
-- admin still corrects it via the Venues tab's Edit dialog, not a re-import.
--
-- Not retroactive: this only fixes venues touched by a future import run
-- that happens to carry a county for them. The 54 already-blank venues from
-- past imports need the Venues tab's Edit action (added alongside this
-- migration) or a one-off manual UPDATE.
